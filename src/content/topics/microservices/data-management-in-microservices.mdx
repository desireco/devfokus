...
featured: true
relatedTopics: ['an-introduction-to-microservices-architecture', 'decomposing-a-monolith-into-microservices', 'microservices-communication-patterns', 'distributed-database-consistency-patterns', 'cqrs-command-query-responsibility-segregation', 'event-sourcing-database-patterns', 'database-scaling-patterns-read-replicas-connection-pooling-and-caching', 'nosql-vs-sql-database-selection-strategy', 'polyglot-persistence-patterns', 'api-composition-patterns']
...
The **database-per-service** pattern ...a core principle of [microservices architecture](/microservices/an-introduction-to-microservices-architecture). It dictates that each microservice should own and manage its own private database. Other services are not allowed to access this database directly; they must interact with the data only through the owning service's public API. This pattern ensures loose coupling between services, allowing them to be developed, deployed, and scaled independently. However, it introduces the significant challenge of how to query data that is spread across multiple databases.
...
- **How it Works**: A higher-level service ...(like an [API Gateway](/system-design/securing-microservices-api-gateways-and-service-meshes) or a dedicated aggregator service)... is responsible for fetching data from multiple downstream services and combining it before returning it to the client. For example, to display an order details page, the aggregator would fetch the order data from the `Order Service` and the customer data from the `User Service` and then join them in memory.
...
- [API Composition Patterns](/topics/api-composition-patterns)
...
- [Polyglot Persistence Patterns](/topics/polyglot-persistence-patterns)
