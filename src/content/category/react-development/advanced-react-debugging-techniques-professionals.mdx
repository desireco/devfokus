...
export function DebugComponent({ name, data, onUpdate }: DebugComponentProps) {
  ...
  useEffect(() => {
    console.group(`🔍 ${name} Component Debug`);
...
export function createDebugContext<T>(
  ...
  function DebugProvider({ children }: { children: React.ReactNode }) {
    ...
    const debugDispatch = (action: any) => {
      ...
      setTimeout(() => {
        console.group(`🔧 Context Action: ${name}`);
        console.log('Action:', action);
        console.log('Previous state:', prevState);
...
export function useMemoryLeakDetector(componentName: string, options: MemoryLeakDetectorOptions = {}) {
  ...
  useEffect(() => {
    ...
    console.log(`📦 ${componentName} mounted. Active instances:`, mountedInstances.current.size);
    ...
    return () => {
      ...
      console.log(`🗑️ ${componentName} unmounted. Active instances:`, mountedInstances.current.size);
...
export function useResourceTracker(componentName: string) {
  ...
  const addResource = useCallback(
    (name: string, cleanup: () => void) => {
      ...
      console.log(`➕ Resource added to ${componentName}:`, name);

      return () => {
        cleanup();
        resources.current.delete(id);
        console.log(`➖ Resource removed from ${componentName}:`, name);
      };
    },
    [componentName]
  );
    ...
    (props: any) => {
      ...
      if (propsHistory.current.length > 0) {
        ...
        if (JSON.stringify(lastProps) === JSON.stringify(props)) {
          console.warn(`🔄 Unnecessary re-render in ${componentName}: Props unchanged`, {
            renderCount: renderCount.current,
            props,
          });
        }
